    ...
    @guest
        <div class="w-full flex items-center sm:items-end flex-col sm:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="flex items-end sm:place-self-end">
                ...
                <a href="{{ route('login') }}" class="w-min select-none whitespace-nowrap px-6 py-4 ml-5 rounded-2xl ease-out duration-100 hover:bg-dark {{ Route::is('login') ? 'bg-dark' : 'bg-card_bg' }}">
                    {{ __('Account') }}
                </a>
            </div>
        </div>
