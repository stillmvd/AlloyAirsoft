    ...
    <x-pages.container class="flex flex-col">
        <div class="flex flex-row justify-between">
            <div class="flex items-center w-[20%]">
                <x-text.link href="{{ route('home') }}" class="text-[1.2rem] text-white font-light">
                    {{ __('Logo') }}
                </x-text.link>
            </div>
            <div class="flex items-end">
                <x-text.link href="{{ route('home') }}" class="font-light mr-4 hover:text-[2.5rem] ease-out duration-[.2s] {{ Route::is('home') ? 'text-[2.5rem] hover:pb-0 text-white' : 'text-[1.2rem] pb-2 hover:pb-0 hover:text-white text-gray-200/75' }}">
                    {{ __('Upcoming') }}
                </x-text.link>
